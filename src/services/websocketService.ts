...
class WebSocketService {
  ...
  // 构造函数：用于初始化 WebSocketService 单例
  // 主要职责：
  // 1. 如果已有实例，直接返回，保证全局只有一个实例
  // 2. 从本地存储读取用户ID和token
  // 3. 异步加载并初始化 Protobuf 定义
  constructor() {
    // 单例检查：若实例已存在则复用
    if (instance) {
  ...
  private async initProtobuf() {
    try {
      ...
      console.log('Protobuf初始化成功，可用消息类型:',
        // proto.nested 是一个对象（包含.proto文件中定义的所有顶层类型）
        // 表示Protobuf定义中的嵌套类型，输出可用的消息类型
  ...
  private adjustHeartbeatInterval(rtt: number) {
    ...
    // 重新停止并重置心跳定时器，不在此处立即发送心跳，
    // 是为了避免在动态调整心跳间隔时突然触发脉冲式或重复发送，
    // 保证下一次心跳严格按照新的间隔执行，从而保持发送节奏稳定
    this.stopHeartbeat(); // 停止现有心跳定时器
    this.baseHeartbeatTimer = setInterval(() => {
      // 检查上次心跳时间，避免频繁发送
      const now = Date.now(); // 当前时间
