...
const initSocketServer = async (server) => {
  ...
  io.on('connection', async (socket) => { // 处理新连接事件
      ...
      try {
        ...
        if (data instanceof Buffer || data instanceof Uint8Array) { // 如果数据是二进制格式
          ...
          try {
            ...
            console.log(`解码 Protobuf 心跳成功:`, heartbeatData); // 输出解码成功的信息
          } catch (protoError) {
            console.error('Protobuf 解码失败:', protoError); // 输出解码失败的错误
          }
        } else if (typeof data === 'object') { // 如果数据是对象格式
          // JSON 格式数据
          heartbeatData = data; // 直接使用 JSON 数据
          console.log(`收到 JSON 心跳:`, heartbeatData); // 输出 JSON 心跳数据
        }
        ...
        if (userInfo) { // 如果用户信息存在
          ...
          console.log(`更新用户 ${userId} 的最后心跳时间为: ${new Date(userInfo.lastHeartbeat).toISOString()}`); // 输出更新信息
        }
        ...
        User.findByIdAndUpdate(userId, { // 更新数据库中的最后活跃时间
          ...
          console.log(`已更新用户 ${userId} 在数据库中的最后活跃时间`); // 输出更新成功的信息
        }).catch(err => {
        ...
        try {
          ...
          console.log(`已向用户 ${userId} 发送心跳响应`); // 输出发送成功的信息
        } catch (responseError) {
          console.error('发送心跳响应失败:', responseError); // 输出发送失败的错误
          // 降级为 JSON
        ...
        console.error('心跳处理错误:', error); // 输出心跳处理错误
      }
    });
    ...
    socket.on('disconnect', async () => { // 监听断开连接事件
      console.log(`用户 ${userId} 已断开连接`); // 输出断开连接的信息
  ...
  setInterval(() => { // 设置定时器
    ...
    userConnections.forEach(async (userInfo, userId) => { // 遍历所有用户连接信息
      if (userInfo.status === 'online' && now - userInfo.lastHeartbeat > heartbeatTimeout) { // 如果用户在线且超时
        ...
        console.log(`用户 ${userId} 心跳超时，标记为离线`); // 输出超时信息
      }
    });
  }, 30000); // 每30秒检查一次
  ...
  // 定期打印连接信息
  setInterval(() => { // 设置定时器
    console.log('当前连接数:', userConnections.size); // 输出当前连接数
